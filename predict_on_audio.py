'''
ADAPTED FROM ITS ORIGINAL VERSION FOUND AT:
...
def load_model(task,weights_path):
...
def compute_output(hcqt, time_grid, freq_grid, task, output_format, threshold,
                   use_neg, save_dir, save_name, model):
...
def main(args):
    ...
    hcqt, freq_grid, time_grid = compute_hcqt(args['audio'])
    ...
    if args['task'] == 'all':
        for task in TASKS:
            ...
            compute_output(
                ...
                args['threshold'], args['use_neg'], args['save_dir'], save_name,args['model'])
    else:
